...
export function AnalyticsDashboard({
  ...
  }, [latestByAccount, prevLatestByAccount, filteredPosts.length]);
  ...
  const publishingTrend = useMemo(() => {
    ...
    return [
      { name: "Image (Görsel)", count: imageCount, percent: Math.round((imageCount / total) * 100) },
      { name: "Video", count: videoCount, percent: Math.round((videoCount / total) * 100) },
      { name: "Text (Metin)", count: textCount, percent: Math.round((textCount / total) * 100) },
      { name: "Link (Bağlantı)", count: linkCount, percent: Math.round((linkCount / total) * 100) },
    ];
  }, [filteredPosts]);
  ...
  const detailedPostPerformance = useMemo(() => {
    return filteredPosts.map((p) => {
      ...
      const reach = latestByAccount.find((s) => s.provider === provider)?.reach
        ? Math.floor((latestByAccount.find((s) => s.provider === provider)?.reach || 10) / Math.max(filteredPosts.filter((fp) => fp.targets?.some((t) => t.socialAccount?.provider === provider)).length, 1))
        : 0;
      const reactions = latestByAccount.find((s) => s.provider === provider)?.likes
        ? Math.floor((latestByAccount.find((s) => s.provider === provider)?.likes || 0) / Math.max(filteredPosts.filter((fp) => fp.targets?.some((t) => t.socialAccount?.provider === provider)).length, 1))
        : 0;
      return {
        id: p.id,
        ...
        date: p.createdAt,
        provider,
        accountName: firstTarget?.socialAccount?.accountName || PLATFORM_LABEL[provider],
        reach,
        engagementRate: reach > 0 ? ((reactions / reach) * 100).toFixed(1) + "%" : "0%",
        reactions,
        shares: 0,
        videoViews: isVideo ? "N/A" : "N/A",
      };
    });
  }, [filteredPosts, latestByAccount]);
  ...
  const topPosts = useMemo(() => filteredPosts.slice(0, 4), [filteredPosts]);
    ...
    <div className="min-w-0 space-y-6">
      ...
      <div className="rounded-2xl border border-ink-200/70 bg-white/90 p-4 shadow-[var(--shadow-soft)]">
        <div className="grid gap-4 lg:grid-cols-2">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-400">Platform Filtresi</p>
          ...
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-400">Hesap Filtresi</p>
            <select
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Toplam Takipçi" value={totals.followers} growth={totals.followerGrowth} hint="Seçili platformlar toplamı" />
        <Metric label="Etkileşim Oranı (% ER)" value={`${totals.engagementRate.toFixed(2)}%`} isString hint="(Beğeni + Yorum) / Gösterim" />
        <Metric label="Toplam Gösterim" value={totals.impressions} growth={totals.impressionGrowth} />
          ...
          <Card.Content>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {publishingTrend.map((t) => (
                <div key={t.name} className="rounded-xl border border-ink-100 bg-ink-50/50 p-3 text-center">
        ...
        <Card>
          ...
          <Card.Content>
            {popularHashtags.length === 0 ? (
              <p className="py-4 text-center text-xs text-ink-400">Gönderilerinizde kullanılan hashtag henüz bulunmuyor.</p>
            ) : (
              <div className="flex flex-wrap gap-1.5 max-h-[100px] overflow-y-auto">
                {popularHashtags.map((h) => (
                  ...
                  >
                    <span>{h.tag}</span>
                    <span className="rounded bg-sky-200/70 px-1 py-0.5 text-[10px] font-bold text-sky-900">{h.count}</span>
                  </span>
                ))}
              </div>
            )}
        ...
        <Card className="xl:col-span-1">
          ...
          <Card.Content className="flex flex-col items-center justify-center">
            {platformDistribution.length === 0 ? (
              <div className="flex h-40 items-center justify-center text-sm text-ink-400">Bu filtrede veri yok</div>
            ) : (
              <div className="h-[220px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={platformDistribution} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={4} dataKey="value">
                      {platformDistribution.map((entry) => (
                        <Cell key={entry.name} fill={PLATFORM_COLOR[entry.providerKey] || "#1a78f5"} />
                      ))}
                    </Pie>
                    <Tooltip />
            ...
            {topPosts.length === 0 ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {topPosts.map((post) => {
                  ...
                  return (
                    <div key={post.id} className="flex items-start gap-3 rounded-xl border border-ink-100 bg-ink-50/50 p-3 transition hover:border-ink-200">
      ...
      <Card>
        <Card.Header>
          ...
          <Card.Description>Tüm gönderilerin erişim, etkileşim, reaksiyon ve video izlenme analizleri</Card.Description>
        </Card.Header>
      ...
      <Card>
        <Card.Header>
          ...
          <Card.Description>{tableRows.length} kayıt · platform / hesap / tarih filtresi uygulandı</Card.Description>
        </Card.Header>
        <Card.Content className="overflow-auto">
          {tableRows.length === 0 ? (
            <p className="py-6 text-center text-sm text-ink-400">Seçilen filtrede veri yok.</p>
          ) : (
      ...
      {/* Fiba Portföy 16:9 Slide Deck Report */}
      {showReportModal ? (
        <FibaSlideReport
          snapshots={snapshots}
          posts={posts}
    ...
    <div className="rounded-2xl border border-ink-200/70 bg-gradient-to-br from-white to-ink-50/80 p-4 shadow-[var(--shadow-soft)]">
      <div className="flex items-center justify-between">
        ...
        {growth !== undefined && growth !== 0 ? (
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${isPositive ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}>
